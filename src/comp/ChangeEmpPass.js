...
function ChangeEmpPass(props) {
    ...
    const handleChangePass = async (e) => {
        ...
        try {
            setLoading(true);
            const response = await axios.post('http://localhost:5000/changeEmpPass', {
                email,
                password,
                key
            });
            ...
            <div className="container position-relative" style={{ backgroundColor: props.mode === 'light' ? 'white' : '#343a40', color: props.mode === 'light' ? 'black' : 'white' }}>
                <form name='adminLogin'>
                    ...
                    <center>
                        ...
                        <Link type="submit" role="button" className="btn mt-3 w-100 btn-info w-50" id="backLogin" to={"/employerLogin"}>Back to Login</Link>
                    </center>
                </form>
            </div>
        </>
