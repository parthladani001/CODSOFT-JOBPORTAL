    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                localStorage.setItem('candidate', JSON.stringify(response.data._id));
            ...
            <div className="container" style={{ backgroundColor: props.mode === 'light' ? 'white' : '#343a40', color: props.mode === 'light' ? 'black' : 'white' }}>
                <form name='adminLogin'>
                    <div className="text-center">
                        <h3> <img className="me-2 mb-2" src={loginLogo} alt="Logo" height="30" width="30" /> Candidate Login </h3>
                    </div>
                    <div className="mb-3">
                    ...
                    <h6 className='my-2'>Forgot Password? &nbsp;
                        <Link className={`${location.pathname === "/changePass" ? "active" : ""}`} to="/changePass">Change Here</Link>
                    </h6>
                    <h6 className='my-2'>Not a user? &nbsp;
                        <Link className={`${location.pathname === "/candidateSignUp" ? "active" : ""}`} to="/candidateSignUp">Create account</Link>
                    </h6>
                    <center>
